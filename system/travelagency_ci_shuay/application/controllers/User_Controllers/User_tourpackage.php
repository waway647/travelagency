...
class User_tourpackage extends CI_Controller {
	...
	public function __construct()
	{
        ...
        $this->load->database();
        $this->load->model('User_tourpackage_model');
	...
	public function show_user_tourpackages()
	{
		...
		$tourpackages = $this->User_tourpackage_model->getTourPackages($keyword); // Implement this method in your model
	...
	public function tourPackages_session()
	{
		...
		$tourpackage = $this->User_tourpackage_model->get_tourpackage_row($tourpackageID_frompost);
	...
	public function part1() 
	{
		...
		if ($passenger_data) {
			...
			$this->User_tourpackage_model->erase_passenger_bin();
		}

		$tourpackagedata = $this->session->userdata('tourpackage_id');
		if(!$tourpackagedata)
		{
			redirect('User_Controllers/User_tourpackage/show_user_tourpackages');
		}   
		else
		{
		...
		public function part1_submit() 
		{
			...
			$data['itineraries'] = $this->User_tourpackage_model->getItinerary($tourpackage_id);
			...
			if (!empty($data['itineraries'])) {
				...
				{
					redirect('User_Controllers/User_tourpackage/part2');
				}
			} 
			else 
			{
				redirect('User_Controllers/User_tourpackage/part1');
			}
		}
	...
	public function part2_passenger_submit() 
	{
		...
		$result = $this->User_tourpackage_model->savepassengerTo_bin($passenger);
		if($result)
		{
		...
		{
			$data['passenger'] = $this->User_tourpackage_model->getpassenger_edit($passenger_id);
		...
		public function part2_passenger_update($passenger_id)
		{
			...
			$result = $this->User_tourpackage_model->updatepassenger($passenger_id,$passenger);
			if($result == true)
			{
				redirect('User_Controllers/User_tourpackage/part2');
			}
		...
		{
			$result = $this->User_tourpackage_model->deletepassenger($passenger_id);
			if($result == true)
			{
				redirect('User_Controllers/User_tourpackage/part2');
			}
	...
	public function part2_contact_submit() 
	{
		...
		if (isset($session_contact['contact_firstName'])) 
		{
			...
			$passengerCount = $this->User_tourpackage_model->getPassengerCount();
			...
			if ($passengerCount > 0) 
			{
				...
				redirect('User_Controllers/User_tourpackage/part3');
			} 
			else 
			{
    ...
    public function part3_submit() {
		...
		$insertContact = $this->User_tourpackage_model->insert_contact($contact);
		...
		$getpassengers = $this->User_tourpackage_model->getpassenger_bin();
	
		if ($getpassengers) {
			// Step 4: Loop through each passenger and insert into the passenger table
			foreach ($getpassengers as $passenger) {
				$insertedPassengerId = $this->User_tourpackage_model->insert_passenger($passenger);
	
				if ($insertedPassengerId) {
					// Step 5: Group passenger and contact
					$this->User_tourpackage_model->group_passenger_contact($insertedPassengerId, $insertContact);
			...
			$insertTour = $this->User_tourpackage_model->insert_tour($tourpackage_id, $tourStartDate, $tourEndDate);
			...
			$this->User_tourpackage_model->insert_booking($insertTour, $insertContact);
		...
		if ($this->db->trans_status() === FALSE) {
			...
			redirect('User_Controllers/User_tourpackage/success');
		}
	}
	...
	public function loadRecord_passenger($rowno = 0)
	{
		...
		$config['base_url'] = base_url() . 'User_Controllers/User_tourpackage/loadRecord_passenger';
	...
	public function loadRecord2($rowno = 0)
	{
		...
		// Pagination configuration
		$config['base_url'] = base_url() . 'User_Controllers/User_tourpackage/loadRecord2';
		$config['use_page_numbers'] = TRUE;
		$config['total_rows'] = $allcount;
		$config['per_page'] = $rowperpage;
		$config['num_links'] = 10;
